...
namespace Nistec.Data.Sqlite
{
    ...
    public abstract class PersistentDictionary<T> //:IDictionary<string,T>
    {
        ...
        protected abstract object GetDataValue(T value);
        protected abstract T DecompressValue(T value);
                ...
                switch (_CommitMode)
                {
                    case CommitMode.OnDisk:
                        using (var db = new DbLite(ConnectionString, DBProvider.SQLite))
                        {
                            ...
                            res = db.ExecuteTransCommandNonQuery(cmdText, DataParameter.Get<SQLiteParameter>("key", key, "body", body), (result, trans) =>
                            {
                                if (result > 0)
                                {
                ...
                switch (_CommitMode)
                {
                    case CommitMode.OnDisk:
                        using (var db = new DbLite(ConnectionString, DBProvider.SQLite))
                        {
                            ...
                            db.ExecuteTransCommandNonQuery(cmdText, DataParameter.Get<SQLiteParameter>("key", key, "body", body), (result, trans) =>
                            {
                                if (result > 0)
                                {
                ...
                switch (_CommitMode)
                {
                    case CommitMode.OnDisk:
                        using (var db = new DbLite(ConnectionString, DBProvider.SQLite))
                        {
                            ...
                            db.ExecuteTransCommandNonQuery(cmdText, DataParameter.Get<SQLiteParameter>("key", key), (result, trans) =>
                            {
                                if (result > 0)
                                {
